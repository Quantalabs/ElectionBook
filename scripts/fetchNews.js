...
const bUrl = `https://api.worldnewsapi.com/search-news?text=Kamala+Harris&language=en&number=50&api-key=${APIKey}&earliest-publish-date=${aweekago.toISOString().split('T')[0]}&sort-direction=DESC&sort=publish-time`;
...
fetch(tUrl, {
	...
	.then(() => {
		fetch(bUrl, {
			...
			.then((response) => response.json())
			.then((data) => {
				fs.writeFileSync('static/harrisNews.json', JSON.stringify(data));
			});
	});
